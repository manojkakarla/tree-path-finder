...
public class Tree {
    private Map<Integer, List<TriangleNode>> nodesLevelMap = new HashMap<Integer, List<TriangleNode>>();

    public void addNodeForLevel(int level, TriangleNode node) {
        List<TriangleNode> triangleNodes = nodesLevelMap.get(level);
        if (triangleNodes == null) {
            triangleNodes = new ArrayList<TriangleNode>(level);
            nodesLevelMap.put(level, triangleNodes);
        }
